...
describe('In the standalone scripts', function () {
  describe('fifa', function () {
    it('works for group G', function(done) {
      exec('coffee fifa2014group.coffee G', function (error, stdout, stderr) {
        lines = stdout.split(/\r?\n/)
        lines[0].should.match(/^Team\s*W\s*D\s*L/)
    ...
    it('errors if no argument', function(done) {
      exec('coffee fifa2014group.coffee', function (error, stdout, stderr) {
        stderr.should.not.be.empty
        done()
      })
    })
    it('errors on argument Asdf', function(done) {
      exec('coffee fifa2014group.coffee Asdf', function (error, stdout, stderr) {
        stderr.should.not.be.empty
        done()
      })
    })
  ...
  describe('lines', function () {
    it('works for a simple test case', function (done) {
      ...
      exec('coffee lines.coffee "' + file + '"' , function (error, stdout, stderr) {
  ...
  describe('wordcount', function () {
    it('works for a simple test case', function(done) {
      exec('echo "QQQ A abc\na  a\tHA:qQq" | coffee wordcount.coffee', function (error, stdout, stderr) {
        lines = stdout.split(/\r?\n/)
